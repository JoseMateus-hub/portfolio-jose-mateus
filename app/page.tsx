...
function Tag({ children }: { children: React.ReactNode }) {
  return (
    <span className="inline-flex items-center gap-1.5 rounded-sm border border-violet-500/20 bg-violet-500/10 px-2.5 py-1 font-mono text-[11px] text-violet-300">
      {children}
    </span>
  );
}
...
export default function PortfolioJoseMateus() {
    ...
    <>
      ...
      <main className="relative min-h-screen overflow-x-hidden bg-[#05040a] font-sans text-white">
        ...
        <div className="pointer-events-none fixed inset-0 z-0">
          ...
          />
          <div className="absolute top-0 left-1/2 h-[500px] w-[500px] -translate-x-1/2 -translate-y-1/2 rounded-full bg-violet-600/10 blur-[120px]" />
          <div className="absolute bottom-1/4 right-0 h-[400px] w-[400px] rounded-full bg-fuchsia-600/8 blur-[100px]" />
          <div className="absolute top-1/2 left-0 h-[300px] w-[300px] rounded-full bg-violet-900/15 blur-[80px]" />
        </div>
        ...
        <header
          className={`fixed left-0 right-0 top-0 z-50 transition-all duration-500 ${
            scrolled
              ? "border-b border-violet-500/10 bg-[#05040a]/85 backdrop-blur-2xl"
              : "bg-transparent"
          }`}
        >
          <nav className="mx-auto flex max-w-6xl items-center justify-between px-6 py-5">
            <a href="#inicio" className="group flex items-center gap-3">
              <div className="relative flex h-9 w-9 items-center justify-center overflow-hidden rounded-sm border border-violet-500/40 bg-violet-500/10">
                <span className="font-mono text-xs font-black text-violet-300 transition group-hover:text-violet-100">
                  JM
                </span>
                <div className="absolute bottom-0 left-0 h-0.5 w-full scale-x-0 bg-violet-500 transition-transform group-hover:scale-x-100" />
              </div>
              <span className="hidden font-mono text-sm text-slate-400 transition group-hover:text-violet-300 sm:block">
                jose_mateus<span className="animate-blink text-violet-400">_</span>
              </span>
            </a>
                  ...
                  <li key={link.href}>
                    <a
                      ...
                      className={`font-mono text-xs uppercase tracking-widest transition-colors ${
                        isActive ? "text-violet-300" : "text-slate-600 hover:text-slate-300"
                      }`}
                    >
                      {isActive && <span className="mr-1 text-violet-500">/</span>}
                      {link.label}
                    </a>
                  </li>
                );
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="hidden items-center gap-2 rounded-sm border border-violet-500/30 bg-violet-500/5 px-4 py-2 font-mono text-xs text-violet-400 transition hover:border-violet-500/60 hover:bg-violet-500/15 hover:text-violet-200 md:flex"
            >
              github ↗
            </a>
            ...
            >
              {[0, 1, 2].map((i) => (
                <span
                  key={i}
                  className={`block h-px w-6 bg-violet-400 transition-all ${
          ...
          <div className={`overflow-hidden transition-all duration-300 md:hidden ${mobileOpen ? "max-h-96" : "max-h-0"}`}>
            <div className="border-t border-violet-500/10 bg-[#05040a]/95 px-6 py-4 backdrop-blur-xl">
              {navLinks.map((l) => (
                <a
                  key={l.href}
                  href={l.href}
                  onClick={() => setMobileOpen(false)}
                  className="block py-3 font-mono text-sm text-slate-400 hover:text-violet-300"
                >
        ...
        >
          <div className="mb-8 flex items-center gap-2 rounded-sm border border-violet-500/20 bg-violet-500/5 px-4 py-2">
          ...
          <div className="relative mb-8">
            <div className="relative flex h-28 w-28 items-center justify-center rounded-sm border border-violet-500/30 bg-[#0d0b17]">
              <span className="absolute top-0 left-0 h-3 w-3 border-l border-t border-violet-500" />
              <span className="absolute top-0 right-0 h-3 w-3 border-r border-t border-violet-500" />
              <span className="absolute bottom-0 left-0 h-3 w-3 border-b border-l border-violet-500" />
              <span className="absolute bottom-0 right-0 h-3 w-3 border-b border-r border-violet-500" />
              <span className="font-mono text-2xl font-black text-violet-300">JM</span>
            </div>
          </div>

          <h1 className="text-5xl font-black tracking-tight md:text-7xl lg:text-8xl">
            <span className="block text-white">José</span>
            <span className="glitch relative block bg-gradient-to-r from-violet-400 via-fuchsia-400 to-violet-600 bg-clip-text text-transparent">
              Mateus
              <span
                aria-hidden
                className="glitch-copy absolute inset-0 bg-gradient-to-r from-violet-400 via-fuchsia-400 to-violet-600 bg-clip-text text-transparent opacity-0"
              />
            </span>
          </h1>
  ...
  <span className="text-violet-300">i9 Publicidade</span>, construindo
  sites institucionais e APIs com{" "}
  <span className="text-violet-300">Node.js</span>,{" "}
  <span className="text-violet-300">TypeScript</span>,{" "}
  <span className="text-violet-300">React</span> e{" "}
  ...
  Boas práticas, testes e arquitetura limpa.
          ...
          <div className="mt-10 flex flex-wrap items-center justify-center gap-4">
            <a
              href="#projetos"
              className="group relative overflow-hidden rounded-sm border border-violet-500/50 bg-violet-500/10 px-8 py-3.5 font-mono text-sm font-semibold text-violet-200 transition-all hover:border-violet-400 hover:bg-violet-500/20 hover:text-white hover:shadow-[0_0_30px_rgba(139,92,246,0.25)]"
            >
              <span className="relative z-10">ver projetos</span>
              <div className="absolute inset-0 translate-x-[-100%] bg-gradient-to-r from-transparent via-violet-500/10 to-transparent transition-transform duration-700 group-hover:translate-x-[100%]" />
            </a>
            <a
              href="#contato"
              className="rounded-sm border border-white/10 px-8 py-3.5 font-mono text-sm text-slate-400 transition hover:border-violet-500/30 hover:text-violet-300"
            >
              contato
            </a>
          </div>
          ...
          <div className="absolute bottom-10 left-1/2 flex -translate-x-1/2 flex-col items-center gap-2 opacity-30">
            ...
            <div className="h-8 w-px animate-pulse bg-gradient-to-b from-violet-500 to-transparent" />
          </div>
        </section>
            ...
            <Reveal delay={100} className="md:col-span-3">
              <div className="space-y-5 leading-8 text-slate-400">
                <p>
                  ...
                  <span className="text-violet-300">back-end Node.js</span> e{" "}
                  <span className="text-violet-300">front-end React/Next.js</span>.
                ...
                <p>
                  ...
                  <span className="text-fuchsia-400">segurança da informação</span> e arquitetura de sistemas —
            ...
            <Reveal delay={200} className="md:col-span-2">
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  <div
                    key={s.l}
                    className="relative overflow-hidden rounded-sm border border-violet-500/15 bg-violet-500/5 p-5"
                  >
                    <div className="absolute right-2 top-2 h-8 w-8 rounded-full bg-violet-500/5 blur-xl" />
                    <p className="font-mono text-3xl font-black text-violet-300">{s.v}</p>
            ...
            <div className="mt-10 grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-4">
              {skills.map((skill, i) => (
                <div
                  ...
                  className="group relative overflow-hidden rounded-sm border border-white/5 bg-white/[0.02] px-4 py-4 transition-all duration-300 hover:border-violet-500/40 hover:bg-violet-500/5"
                >
                  <div className="absolute bottom-0 left-0 h-px w-0 bg-violet-500 transition-all duration-300 group-hover:w-full" />
                  <span className="font-mono text-base text-violet-500/40 group-hover:text-violet-400">
                    {skill.icon}
                  </span>
          ...
          <div className="mt-10 grid gap-5 md:grid-cols-2">
            {projects.filter((p) => p.featured).map((p, i) => (
              <Reveal key={p.title} delay={i * 100}>
                <article className="group relative h-full overflow-hidden rounded-sm border border-violet-500/15 bg-[#0d0a18] p-8 transition-all duration-300 hover:border-violet-500/40 hover:shadow-[0_0_40px_rgba(139,92,246,0.08)]">
                  <div className="absolute left-0 top-0 h-px w-0 bg-gradient-to-r from-violet-500 to-fuchsia-500 transition-all duration-500 group-hover:w-full" />
                  <div className="pointer-events-none absolute right-4 top-4 opacity-0 transition-opacity group-hover:opacity-100">
                    <span className="font-mono text-xs text-violet-700">[{String(i + 1).padStart(2, "0")}]</span>
                  </div>

                  <div className="mb-6 flex h-12 w-12 items-center justify-center rounded-sm border border-violet-500/20 bg-violet-500/10 font-mono text-xl text-violet-400">
                    {p.glyph}
                  </div>
                  ...
                  <div className="mt-5 flex flex-wrap gap-1.5">
                    {p.tech.map((t) => (
                      <span key={t} className="rounded-sm border border-violet-500/20 bg-violet-500/8 px-2 py-0.5 font-mono text-[11px] text-violet-400">
                        {t}
                      </span>
                    ))}
                  </div>
                  ...
                  <div className="mt-7">
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="inline-flex items-center gap-2 font-mono text-xs text-slate-500 transition hover:text-violet-400"
                    >
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {projects.filter((p) => !p.featured).map((p, i) => (
              <Reveal key={p.title} delay={i * 70}>
                <article className="group h-full rounded-sm border border-white/[0.05] bg-white/[0.01] p-5 transition-all hover:border-violet-500/20 hover:bg-violet-500/[0.03]">
                  <span className="font-mono text-xl text-violet-700 group-hover:text-violet-500">
                    {p.glyph}
                  </span>
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="mt-4 inline-flex items-center gap-1 font-mono text-[11px] text-slate-700 transition hover:text-violet-500"
                  >
                    código ↗
                  </a>
                </article>
          ...
          <div className="mt-10 space-y-3">
            {experiences.map((exp, i) => (
              <Reveal key={exp.role + exp.company} delay={i * 80}>
                <div className="group relative overflow-hidden rounded-sm border border-white/[0.05] bg-white/[0.01] p-6 transition-all hover:border-violet-500/20 hover:bg-violet-500/[0.03]">
                  <div className="absolute left-0 top-0 h-full w-px origin-top scale-y-0 bg-violet-500 transition-transform duration-300 group-hover:scale-y-100" />

                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      <h3 className="font-mono font-bold text-white">{exp.role}</h3>
                      <p className="mt-1 font-mono text-sm text-violet-400/70">
                        {exp.company}
                    ...
                    <div className="flex items-center gap-2">
                      {exp.current && (
                        <span className="rounded-sm border border-violet-500/30 bg-violet-500/10 px-2.5 py-0.5 font-mono text-[10px] uppercase tracking-wider text-violet-400">
                          atual
                        </span>
                      )}
        ...
        <section id="contato" className="relative z-10 mx-auto max-w-6xl px-6 py-28">
          <Reveal>
            <div className="relative overflow-hidden rounded-sm border border-violet-500/20 bg-[#0d0a18] p-10 md:p-16">
              <div className="pointer-events-none absolute -right-20 -top-20 h-72 w-72 rounded-full bg-violet-600/10 blur-[80px]" />
              <div className="pointer-events-none absolute -bottom-10 -left-10 h-40 w-40 rounded-full bg-fuchsia-600/8 blur-[60px]" />
              <span className="absolute left-6 top-6 font-mono text-xs text-violet-800">{"/*"}</span>
              <span className="absolute bottom-6 right-6 font-mono text-xs text-violet-800">{"*/"}</span>
              ...
              <p className="mt-4 max-w-lg leading-8 text-slate-500">
                ...
                Respondo rápido — pode mandar mensagem.
              </p>

              <div className="mt-10 grid gap-3 sm:grid-cols-3">
                <button
                  onClick={copyEmail}
                  className="group flex items-start gap-4 rounded-sm border border-white/[0.06] bg-white/[0.02] p-5 text-left transition hover:border-violet-500/30 hover:bg-violet-500/5"
                >
                  <div className="mt-0.5 flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-sm border border-violet-500/20 bg-violet-500/10 font-mono text-violet-400">
                    @
                  </div>
                  <div>
                    <p className="font-mono text-[10px] uppercase tracking-wider text-slate-600">e-mail</p>
                    <p className="mt-1 break-all font-mono text-xs text-slate-300 transition group-hover:text-violet-300">
                ...
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="group flex items-start gap-4 rounded-sm border border-white/[0.06] bg-white/[0.02] p-5 transition hover:border-violet-500/30 hover:bg-violet-500/5"
                >
                  ...
                  <div>
                    ...
                    <p className="mt-1 font-mono text-xs text-slate-300 transition group-hover:text-violet-300">
                      JoseMateus-hub ↗
                    </p>
                  </div>
                </a>
